...
class BacklogService:
    ...
    def add(
        ...
        item = BacklogItem(
            ...
            source_id=source_id, depends_on=depends_on,
        )
    ...
    def update_status(self, backlog_id: int, status: str, conn=None) -> dict:
        """Update backlog item status. Returns warning if dependency not done."""
        repo = self._get_repo(conn)
        item = repo.get(backlog_id)
        if not item:
            return {}
        ...
        warning = None
        if status == "in_progress" and item.depends_on:
            dep = repo.get(item.depends_on)
            if dep and dep.status != BacklogStatus.DONE:
                warning = f"Dependency #{item.depends_on} ({dep.title}) is not done (status: {dep.status.value})"

        repo.save(item)
        return {"warning": warning} if warning else {}

    def update_depends_on(self, backlog_id: int, depends_on: Optional[int], conn=None) -> None:
        """Update backlog item dependency."""
        repo = self._get_repo(conn)
        item = repo.get(backlog_id)
        if not item:
            return
