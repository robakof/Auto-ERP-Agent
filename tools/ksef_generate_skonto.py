"""Generuje KSeF FA(3) KOR XML dla korekt skontowych (FSK, ZwrNumer=0) z ERP XL.

CLI:
    py tools/ksef_generate_skonto.py --gid 1
    py tools/ksef_generate_skonto.py --gid 1 2
    py tools/ksef_generate_skonto.py --date-from 2026-04-01 --date-to 2026-04-22
    py tools/ksef_generate_skonto.py --validate output/schemat_FA3.xsd --gid 1
    py tools/ksef_generate_skonto.py --dry-run --gid 1
...
from core.ksef.adapters.erp_reader import ErpReader, SQL_PATH_FSK_SKONTO
...
def main() -> int:
    ...
    for k in korekty:
        ...
        n_poz = len(k.stan_przed.wiersze) + len(k.stan_po.wiersze)
        print(f"  [OK] {k.numer_faktury} ({n_poz} poz.) -> {out_path.name}")
        if xsd_path:
            _validate_and_report(xml, xsd_path, k.numer_faktury, out_path.name, errors)

    print(f"\nWygenerowano {len(korekty)} korekt(y) skontowych w {out_dir}")
    return 2 if errors else 0


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Generuj KSeF FA(3) KOR XML z ERP XL (korekty skontowe)")
...
def _compose_filename(numer: str, data) -> str:
    ...
    return f"ksef_skonto_{safe}_{data.isoformat()}.xml"
