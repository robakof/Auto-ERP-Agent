...
def diag(nip: str, nazwa: str | None = None) -> None:
    ...
    if nazwa:
        print()
        print(f"Szukam: {repr(nazwa)}")
        cur.execute(_MATCH_SQL, [knt_numer, nazwa])
        match = cur.fetchone()
        if match:
            print(f"  [OK] Znaleziono Twr_Kod = {match[0]}")
        else:
            print(f"  [BRAK] Brak dopasowania (case-insensitive, trim).")
            print(f"  Sprawdź czy jeden z wpisów powyżej odpowiada tej nazwie.")
...
def main() -> None:
    ...
    parser.add_argument("--nazwa", help="Nazwa z faktury (P_7) do przetestowania dopasowania")
    args = parser.parse_args()
    diag(args.nip, args.nazwa)
