"""Testy dla tools/xl_invoice_bulk.py."""
...
import tools.xl_invoice_bulk as xb
from tools.xl_invoice_parser import FzInvoice, FzPozycja

_POZ = FzPozycja(nr=1, nazwa="Surowiec A", ilosc=Decimal("10"), jm="szt",
...
class TestBulkImport:
    ...
    def test_report_status_blad_on_parse_error(self, tmp_path):
        _xml_dir(tmp_path)
        report = tmp_path / "raport.xlsx"
        with patch.object(xb, "parse_ksef_xml", return_value=_PARSE_FAIL):
            xb.bulk_import(tmp_path, report=report)
