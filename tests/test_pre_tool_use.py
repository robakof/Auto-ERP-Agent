...
class TestLifecycleHookPassthrough:
    """spawn/stop/resume pass through hook — policy routing is in the tool itself."""

    def test_spawn_allowed(self):
        rc, out = run_hook(make_bash("py tools/agent_bus_cli.py spawn --role erp_specialist --task test"))
        assert out["hookSpecificOutput"]["permissionDecision"] == "allow"

    def test_spawn_request_allowed(self):
        rc, out = run_hook(make_bash("py tools/agent_bus_cli.py spawn-request --role erp_specialist --task test"))
        assert out["hookSpecificOutput"]["permissionDecision"] == "allow"

    def test_stop_allowed(self):
        rc, out = run_hook(make_bash("py tools/agent_bus_cli.py stop --session abc123"))
        assert out["hookSpecificOutput"]["permissionDecision"] == "allow"

    def test_resume_allowed(self):
