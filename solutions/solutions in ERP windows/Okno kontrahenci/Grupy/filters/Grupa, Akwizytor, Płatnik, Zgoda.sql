...
@PAR ?@R(SELECT '(wszystkie)' AS KOD,'all' AS ID UNION ALL SELECT 'Zła grupa' AS KOD,'tak' AS ID UNION ALL SELECT 'Poprawna' AS KOD,'nie' AS ID)|ZlaGrupa|&Zła grupa główna:REG=all @? PAR@
...
AND (??_QZlaGrupa='all'
  OR (??_QZlaGrupa='tak' AND EXISTS (SELECT 1 FROM CDN.KntGrupy mg WHERE mg.KnG_GIDNumer=Knt_GIDNumer AND mg.KnG_GIDTyp=32 AND EXISTS (SELECT 1 FROM CDN.KntGrupy child WHERE child.KnG_GrONumer=mg.KnG_GrONumer AND child.KnG_GIDTyp=-32)))
  OR (??_QZlaGrupa='nie' AND NOT EXISTS (SELECT 1 FROM CDN.KntGrupy mg WHERE mg.KnG_GIDNumer=Knt_GIDNumer AND mg.KnG_GIDTyp=32 AND EXISTS (SELECT 1 FROM CDN.KntGrupy child WHERE child.KnG_GrONumer=mg.KnG_GrONumer AND child.KnG_GIDTyp=-32))))