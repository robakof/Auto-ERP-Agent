...
@PAR ?@R(SELECT '(wszystkie)' AS KOD,0 AS ID UNION ALL SELECT TrP_FormaNazwa,TrP_FormaNr FROM cdn.TraPlat GROUP BY TrP_FormaNazwa,TrP_FormaNr ORDER BY 1)|FormaPl|&Forma płatności:REG=0 @? PAR@
...
AND (??FormaPl=0 OR EXISTS(SELECT 1 FROM cdn.TraPlat WHERE TrP_GIDNumer=TrN_GIDNumer AND TrP_GIDTyp=TrN_GIDTyp AND TrP_FormaNr=??FormaPl))
