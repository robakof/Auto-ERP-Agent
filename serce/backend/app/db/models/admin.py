...
class ContentFlag(Base):
    ...
    target_type: Mapped[FlagTargetType] = mapped_column(
        Enum(FlagTargetType, values_callable=enum_values), nullable=False
    )
    target_id: Mapped[uuid.UUID] = mapped_column(nullable=False)
    reason: Mapped[FlagReason] = mapped_column(
        Enum(FlagReason, values_callable=enum_values), nullable=False
    )
    description: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    status: Mapped[FlagStatus] = mapped_column(
        Enum(FlagStatus, values_callable=enum_values),
    ...
    resolution_action: Mapped[ResolutionAction | None] = mapped_column(
        Enum(ResolutionAction, values_callable=enum_values), nullable=True
    )
...
class AdminAuditLog(Base):
    ...
    target_type: Mapped[AuditTargetType] = mapped_column(
        Enum(AuditTargetType, values_callable=enum_values), nullable=False
    )
