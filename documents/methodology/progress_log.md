...
### 2026-03-08 — Sesja deweloperska — bugi + refleksje agenta

Zrealizowano backlog refleksji agenta i zgłoszone bugi:

**Bugi naprawione:**
- SqlClient.validate(): `_split_statements()` — obsługa średników w string literalach (`''`)
- SqlClient.validate(): `_strip_comments()` — usuwanie linii `--` przed walidacją
- Encoding cp1250: `tools/lib/output.py` → `print_json()` z `sys.stdout.reconfigure(utf-8)`; wdrożone we wszystkich 10 toolsach

**Refleksje agenta wdrożone:**
- AI_GUIDELINES.md: sekcja "Komendy powłoki" — zakaz `$()`, `python -c`, maks. 2 `&&`, reguła Edit z minimalnym kontekstem
- ERP_SCHEMA_PATTERNS.md: uproszczona sekcja "Odkrywanie nazw nieznanych tabel" — zasada `docs_search "[prefiks]GIDNumer"` + przykład
- e_typy.html → docs.db: `parse_gid_types()` (HTMLParser), tabele `gid_types` + `gid_types_fts` (FTS5); `docs_search` zwraca `data.gid_types[]`; 456 typów zaindeksowanych; wyszukiwanie po symbolu (ZPZ) i numerze (14346)

**Struktury:**
- `solutions/bi/drafts/` — usunięto (zawartość przeniesiona do `solutions/bi/Rezerwacje/`)

**Testy:** 202 zielone (było 191, +11 nowych dla gid_types)

**Pozostałe w backlogu refleksji:**
