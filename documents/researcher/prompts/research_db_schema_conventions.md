...
Projekt wykorzystuje SQLite (single-file DB, WAL mode) jako centralną bazę danych dla systemu wieloagentowego. Baza ma 14+ tabel i rośnie (docelowo 50+). Wielu agentów (procesów) czyta i pisze jednocześnie. Nie używamy ORM — SQL bezpośrednio w Pythonie. Chcemy stworzyć konwencję schematu bazy, opartą na sprawdzonych wzorcach.

Draft konwencji istnieje, ale nie był konfrontowany z zewnętrznymi best practices. Potrzebujemy researchu żeby zwalidować lub skorygować nasze wybory.
...
1. **Naming conventions dla SQLite** — plural vs singular table names, snake_case vs camelCase. Czy jest konsensus w community? Jak robią to znane projekty open-source używające SQLite? (np. Firefox, Android, Homebrew, Django z SQLite backend)

2. **Migration patterns dla SQLite single-file DB** — jakie są dojrzałe wzorce migracji schematów w projektach bez ORM? Alembic? Własna tabela `schema_migrations`? Numbered SQL files? Inline `ALTER TABLE` w kodzie? Trade-offy każdego podejścia. Szczególnie: co działa dobrze gdy baza jest mała (1 plik) ale schema rośnie szybko?

3. **Multi-agent/multi-process concurrent access do SQLite** — best practices poza WAL i busy_timeout. Jakie inne PRAGMAs, connection settings, wzorce architektoniczne pomagają przy wielu procesach (50-100+) czytających i piszących jednocześnie? Connection pooling? Read-only connections? Write queue? Sharding?

4. **CHECK constraints vs application-level validation** — kiedy CHECK w DB, a kiedy walidacja w kodzie? Trade-offy (performance, czytelność, utrzymywalność). Szczególnie: czy CHECK na każdym polu enum to overkill, czy standard?

5. **Index naming conventions** — wzorzec `idx_table_columns` vs inne. Jak robią to dojrzałe projekty? Czy istnieje konsensus? Jak nazywać indeksy: unique, composite, partial?

## Output contract

Zapisz wyniki do: `documents/researcher/research/research_results_db_schema_conventions.md`
