...
### Pattern: Convention First Architecture

**Problem:** System powstaje bez konwencji — każdy component ma własną strukturę. Konsekwencja: refactor all components gdy chcemy zmienić architekturę (np. migrate do DB-driven orchestration).

**Solution:**
- **Zacznij od convention** przed implementacją pierwszego component
- Convention definiuje contract (co MUST have, co optional)
- Wszystkie components przestrzegają convention (consistency)
- Pattern: `convention → template → implementation`

**When to use:**
- Powtarzalny aspekt systemu (workflows, API endpoints, tests, configs)
- Multiple authors (Developer, PE, ERP Specialist) tworzą podobne artifacts
- Future architecture known (np. DB-driven, event-driven)
- System będzie skalował (10+ components tego samego typu)

**When NOT to use:**
- Proof of concept (struktura się zmienia)
- Single component (over-engineering)
- Temporary/experimental (stabilizuj najpierw)

**Example:**
```
Problem (NO convention):
  PE tworzy workflow → czyta inny workflow jako wzorzec
  Każdy workflow ma inną strukturę (różne sections, różna granularity)
  User chce: workflow as data (DB-driven orchestration)
  Konsekwencja: refactor ALL workflows (bo nie było convention)

Solution (Convention First):
  1. Utwórz WORKFLOW_CONVENTION.md (contract)
     - MUST HAVE: metadata header (YAML), steps format, exit gates
     - OPTIONAL: decision points, examples
     - Future-proof: parseable do DB schema

  2. Template (example implementation)
     - Pokazuje jak convention wygląda w praktyce

  3. Wszystkie workflows przestrzegają convention
     - PE używa template (nie czyta starych workflows)
     - Consistency (każdy workflow ma metadata, steps, exit gates)

  4. Future migration (workflow → DB):
     - Parsowanie trivial (struktura już zgodna)
     - Zero refactor (convention była DB-ready od początku)

Result:
  - 10 workflows → 1 parser (convention-driven)
  - Future-proof (DB migration bez refactor)
  - Onboarding szybki (developer czyta convention, rozumie wszystkie workflows)
```

**Anti-pattern:**
```
Wzorowanie się na istniejącym component zamiast convention:
  ✗ PE czyta ERP_COLUMNS_WORKFLOW jako wzorzec
  ✗ Kopiuje strukturę (może być przestarzała/nieoptymalna)
  ✗ Nowy workflow różni się od innych (inconsistency)
  ✗ Brak forward compatibility (nie DB-ready)

Instead:
  ✓ PE czyta WORKFLOW_CONVENTION.md (canonical)
  ✓ Używa template (current best practice)
  ✓ Nowy workflow zgodny z resztą (consistency)
  ✓ Future-proof (convention zaprojektowana dla DB migration)
```

**Enforcement:**
- Convention MUST BE canonical (PE/Developer czytają convention, NIE stare components)
- Template MUST BE up-to-date (sync z convention)
- Review gate: przed merge sprawdź czy component przestrzega convention

**Ref:**
- Workflow convention request (user observation 2026-03-24)
- WORKFLOW_CONVENTION.md: `documents/prompt_engineer/WORKFLOW_CONVENTION.md`
- Pattern discovered: Architect code walkthrough session
